...
import { 
  ...
  Upload, Image as ImageIcon, Edit, GraduationCap, BookOpen, Layers, Send
} from "lucide-react";
...
export default function AdminFormations() {
  ...
  // MODIFICATION : On initialise à "all" par défaut comme sur AdminStagiaires
  ...
  // MODIFICATION : Mise à jour de la logique de filtrage pour supporter "all"
  const filteredFormations = useMemo(() => {
    return formations.filter((f) => {
      const matchesSearch = f?.nom?.toLowerCase().includes(search.toLowerCase().trim());
      const matchesSemestre = activeSemestre === "all" ? true : f?.semestres?.some(s => s.numero === activeSemestre);
  ...
  return (
    <div className="container mx-auto flex flex-col gap-4 p-3 md:p-5 md:gap-6 h-[calc(100dvh-70px)] md:h-[calc(100vh-80px)] overflow-hidden bg-background">
        ...
        <div className="relative z-10 flex flex-col justify-center w-full h-full gap-3 md:flex-row md:items-center md:justify-between">
          <div className="flex flex-col justify-center flex-1">
            ...
            <p className="max-w-sm text-xs font-medium leading-relaxed md:text-sm text-white/80 md:max-w-full">
              Gérez les spécialités et les modules de formation de l'institut.
            </p>
          </div>
          ...
          >
            <Plus size={18} strokeWidth={3} /> <span>Nouvelle Formation</span>
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-col flex-1 min-h-0 overflow-hidden bg-white border shadow-sm border-black/5 rounded-2xl md:rounded-3xl">
        ...
        {/* ===== BARRE D'OUTILS ET FILTRES (HARMONISÉE) ===== */}
        <div className="flex flex-col justify-between gap-3 p-3 bg-white border-b xl:flex-row xl:items-center md:p-4 shrink-0 border-black/5">
            ...
            {/* GROUPE GAUCHE : Recherche (Réduite) + Bouton Mobile */}
            <div className="flex items-center flex-1 w-full gap-3 xl:w-auto">
            ...
            <div className="flex items-center justify-between w-full xl:w-auto gap-4 overflow-x-auto [scrollbar-width:none]">
              ...
              {/* Sélecteur de Semestres (Exactement comme AdminStagiaires) */}
              ...
              <div className="hidden sm:block px-3 py-2 text-[10px] md:text-xs font-bold tracking-widest text-black/50 uppercase bg-black/[0.02] border border-black/5 rounded-xl whitespace-nowrap shrink-0">
                {filteredFormations.length} <span className="hidden xl:inline">Formation(s)</span>
              </div>
            </div>
        </div>

        {/* ===== LISTE DES FORMATIONS (ACCORDÉON) ===== */}
          ...
          {loading ? (
            ...
            <div className="flex flex-col gap-3">
              {filteredFormations.map((formation) => {
                ...
                // Récupération intelligente des modules selon que "all" est actif ou non
                const modulesToList = activeSemestre === "all" 
                  ? (formation.semestres || []).reduce((acc, sem) => [...acc, ...(sem.modules || []).map(m => ({...m, semestre_numero: sem.numero}))], [])
                  : formation.semestres?.find(s => s.numero === activeSemestre)?.modules || [];
                  ...
                  <div key={formation.id} className="flex flex-col overflow-hidden transition-all duration-300 bg-white border border-black/5 rounded-2xl hover:border-primary/30 hover:shadow-sm">
                    <div onClick={(e) => toggleExpand(e, formation.id)} className="flex items-center gap-3 p-3 cursor-pointer md:p-4 group">
                      ...
                      <div className="flex flex-col justify-center flex-1 min-w-0">
                        ...
                        <p className="text-[10px] md:text-xs font-medium text-black/50 truncate mt-0.5">
                          {modulesToList.length} module(s) {activeSemestre !== "all" ? `en Semestre ${activeSemestre}` : "au total"}
                        </p>
                      </div>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <div className="flex items-center gap-1 transition-opacity opacity-100 md:opacity-0 group-hover:opacity-100">
                          ...
                          <button onClick={(e) => deleteFormation(e, formation.id)} className="p-2 text-black transition-colors rounded-lg bg-black/5 hover:text-red-50 hover:bg-red-50" title="Supprimer"><Trash2 size={14} /></button>
                        </div>
                    ...
                    {expanded === formation.id && (
                      <div className="p-4 pt-0 border-t md:p-5 border-black/5 bg-black/[0.01] animate-in slide-in-from-top-2">
                        ...
                        <div className="flex items-center gap-2 mb-3 text-[10px] font-bold tracking-widest text-primary uppercase">
                          <Layers size={14} /> Modules {activeSemestre !== "all" ? `- Semestre ${activeSemestre}` : "- Tous les semestres"}
                        </div>
                        {modulesToList.length > 0 ? (
                            ...
                            {modulesToList.map((mod) => (
                              <div key={mod.id} className="flex items-center gap-2.5 p-3 bg-white border border-black/5 rounded-xl">
                                ...
                                <span className="text-xs font-bold leading-tight text-black/80">
                                  {mod.nom}
                                  {activeSemestre === "all" && mod.semestre_numero && (
                                    <span className="ml-1.5 px-1.5 py-0.5 rounded text-[9px] bg-black/5 text-black/50 font-bold">
                                      S{mod.semestre_numero}
                                    </span>
                                  )}
                                </span>
                              </div>
                          ...
                          <p className="p-3 text-[10px] italic text-center text-black/40 border border-dashed border-black/10 rounded-xl bg-white">
                            Aucun module trouvé.
                          </p>
                        )}
                      </div>
            ...
            <div className="flex flex-col items-center justify-center h-full py-12 text-center border-2 border-dashed border-black/5 rounded-3xl bg-black/[0.02]">
              ...
              <p className="mt-1 text-xs text-black/30">Aucune formation trouvée.</p>
            </div>
          )}
        </div>
      </div>

      {/* ===== MODAL FORMULAIRE (Reste identique mais stylisé) ===== */}
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in" onClick={closeModal}>
          <div className="w-full max-w-2xl bg-white border border-black/5 shadow-2xl rounded-[2rem] overflow-hidden flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-5 border-b md:px-8 border-black/5 bg-black/[0.01]">
              ...
              <button onClick={closeModal} className="p-2 transition-colors rounded-full text-black/40 hover:text-black hover:bg-black/5"><X size={20}/></button>
            </div>
            ...
            <form id="formationForm" onSubmit={handleSave} className="p-6 md:p-8 space-y-6 overflow-y-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
              <div>
                <label className="block ml-1 mb-2 text-[10px] font-bold text-black/40 uppercase tracking-widest">Image de couverture</label>
              ...
              <div className="flex flex-col gap-5">
                <div className="space-y-1.5">
                  <label className="block ml-1 text-[10px] font-bold text-black/40 uppercase tracking-widest">Intitulé de la formation</label>
                  <input 
                    type="text" required 
                    value={formData.nom} 
                    onChange={e => setFormData({ ...formData, nom: e.target.value })} 
                    className="w-full px-4 py-3.5 text-sm font-bold transition-all bg-black/[0.03] border-none outline-none rounded-2xl focus:ring-2 focus:ring-primary/20" 
                    placeholder="Ex: Master Audiovisuel..." 
                  />
                </div>
                
                <div className="flex flex-col">
                  <label className="block ml-1 mb-1.5 text-[10px] font-bold text-black/40 uppercase tracking-widest">Description</label>
                  <div className="overflow-hidden transition-all bg-white border border-black/10 rounded-2xl focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20">
                    <ReactQuill 
                      theme="snow" 
              ...
              <div className="pt-6 border-t border-black/5">
                 <div className="flex flex-col gap-4 mb-4 sm:flex-row sm:items-center sm:justify-between">
                    <label className="ml-1 text-[10px] font-bold text-black/40 uppercase tracking-widest">Modules du programme</label>
                    <div className="flex gap-1 p-1 overflow-x-auto border bg-black/[0.02] border-black/5 rounded-xl [scrollbar-width:none]">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <button key={s} type="button" onClick={() => setModalActiveSemestre(s)} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all whitespace-nowrap ${modalActiveSemestre === s ? "bg-white text-primary shadow-sm border border-black/5" : "text-black/50 hover:text-black hover:bg-black/5"}`}>S{s}</button>
                      ))}
                    </div>
                 </div>
                 ...
                 <div className="space-y-3">
                   {formData.modules.filter(mod => mod.semestre_numero === modalActiveSemestre).map((mod, idx) => (
                       <div key={mod.id || idx} className="flex items-center gap-3 p-2 pl-4 transition-colors bg-black/[0.02] rounded-xl group hover:border-primary/20 hover:bg-white border border-black/5">
            ...
            <div className="flex gap-3 p-6 mt-auto border-t border-black/5 bg-black/[0.01]">
              <button type="button" onClick={closeModal} className="flex-1 py-4 text-sm font-bold text-black transition-colors bg-white border shadow-sm border-black/5 rounded-2xl hover:bg-black/5">
                Annuler
              </button>
              <button type="submit" form="formationForm" disabled={saving} className="flex items-center justify-center flex-1 gap-2 py-4 text-sm font-bold text-white transition-all shadow-lg bg-primary rounded-2xl shadow-primary/20 hover:opacity-90 active:scale-95 disabled:opacity-70">
